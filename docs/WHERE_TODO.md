...
- Get Cache assets live in `rapper/gc`; the child imports `$gc/...`.
...
- [ ] **Snake ruler as the polygon draw tool.** `getCache_OfflineMap/lib/mapUi/SnakeRuler.svelte` needs `ports: MapHostPorts` and a sibling-child edge declared in `DECLARED_CHILD_DEPS` (`childBoundary.test.ts`); decide whether it replaces `MapDrawControls` or sits beside it.
